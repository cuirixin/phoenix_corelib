...
func NewAliPay(config *Config) *AliPay {
	// 读取
	publickKey, err := ioutil.ReadFile(config.PublicKeyCertPath)
	check(err)
	privateKey, err := ioutil.ReadFile(config.PrivateKeyCertPath)
	check(err)
	aliPublicKey, err := ioutil.ReadFile(config.AliPublicKeyCertPath)
	check(err)
	// println(string(publickKey))
	client := New(config.AppId, config.PartnerId, publickKey, privateKey, config.IsProduction)
